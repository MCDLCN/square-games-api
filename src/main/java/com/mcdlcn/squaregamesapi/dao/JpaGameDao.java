...
public class JpaGameDao implements GameDao {
    ...
    @Override
    public Collection<StoredGame> findAll() throws InconsistentGameDefinitionException {
        List<StoredGame> games = new ArrayList<>();

        for (GameEntity entity : repository.findAll()) {
            games.add(toStoredGame(entity));
        }
    ...
    public Optional<StoredGame> findById(UUID gameId) throws InconsistentGameDefinitionException {
        Optional<GameEntity> entity = repository.findById(gameId);

        if (entity.isEmpty()) {
            return Optional.empty();
        }

        return Optional.of(toStoredGame(entity.get()));
    }
